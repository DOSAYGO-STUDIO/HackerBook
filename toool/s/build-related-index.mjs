...
const RELATED_WORK_SCHEMA_VERSION = "v10_semantic_relax_surprise_edges";
...
const DEFAULTS = {
  ...
  minEdgeWeight: 0.18,
  maxEdgesPerSource: 5,
  semanticRelaxWeight: 0.35,
  surpriseMaxLexical: 0.08,
  surpriseMinSemantic: 0.20,
  surpriseMinWeight: 0.18,
  surpriseLimit: 1500,
  targetMb: 15,
...
Options:
  ...
  --semantic-relax-weight N Blend in semantic support when lexical overlap is low (0..1, default: ${DEFAULTS.semanticRelaxWeight})
  --surprise-max-lexical N Max lexical overlap for "surprising" links (0..1, default: ${DEFAULTS.surpriseMaxLexical})
  --surprise-min-semantic N Min semantic support for "surprising" links (0..1, default: ${DEFAULTS.surpriseMinSemantic})
  --surprise-min-weight N  Min edge weight for "surprising" links (default: ${DEFAULTS.surpriseMinWeight})
  --surprise-limit N       Max surprising links in top index (default: ${DEFAULTS.surpriseLimit})
...
function parseArgs(argv) {
  ...
  if (out["semantic-relax-weight"] != null) out.semanticRelaxWeight = Number(out["semantic-relax-weight"]);
  if (out["surprise-max-lexical"] != null) out.surpriseMaxLexical = Number(out["surprise-max-lexical"]);
  if (out["surprise-min-semantic"] != null) out.surpriseMinSemantic = Number(out["surprise-min-semantic"]);
  if (out["surprise-min-weight"] != null) out.surpriseMinWeight = Number(out["surprise-min-weight"]);
  if (out["surprise-limit"] != null) out.surpriseLimit = Number(out["surprise-limit"]);
...
function initWorkDb(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS edge_scores (
      ...
      lexical_sum REAL NOT NULL DEFAULT 0,
      semantic_sum REAL NOT NULL DEFAULT 0,
  ...
  ensureColumn(db, "edge_scores", "lexical_sum", "REAL NOT NULL DEFAULT 0");
  ensureColumn(db, "edge_scores", "semantic_sum", "REAL NOT NULL DEFAULT 0");
  ensureColumn(db, "edge_scores", "novelty_sum", "REAL NOT NULL DEFAULT 0");
}
...
async function phaseScoreEdges(ctx) {
  ...
  const semanticRelaxWeight = clamp(Number(args.semanticRelaxWeight || 0), 0, 1);
  ...
  if (lastEvidenceId === 0 && processedEvidence === 0) {
    workDb.exec("DELETE FROM edge_scores");
    emittedEdges = 0;
    setMeta(workDb, "score_emitted_edges", "0");
  }
  const startTs = Date.now();
  ...
  const upsertEdge = workDb.prepare(`
    INSERT INTO edge_scores(
      source_story_id, target_story_id, weight, source_align, self_consistency, lexical_sum, semantic_sum, novelty_sum, evidence_count, last_time
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
    ON CONFLICT(source_story_id, target_story_id) DO UPDATE SET
      ...
      lexical_sum = edge_scores.lexical_sum + excluded.lexical_sum,
      semantic_sum = edge_scores.semantic_sum + excluded.semantic_sum,
      novelty_sum = edge_scores.novelty_sum + excluded.novelty_sum,
  ...
  while (true) {
    ...
    for (const row of batch) {
      ...
      const targetPeerSums = new Array(targetTokens.length).fill(0);
      if (uniqTargets.length > 1) {
        let pairSum = 0;
        let pairCount = 0;
        for (let i = 0; i < targetTokens.length; i += 1) {
          for (let j = i + 1; j < targetTokens.length; j += 1) {
            const pairSim = jaccardTokens(targetTokens[i], targetTokens[j]);
            pairSum += pairSim;
            targetPeerSums[i] += pairSim;
            targetPeerSums[j] += pairSim;
            pairCount += 1;
          }
        }
      ...
      const listPenalty = (linkDumpCap > 0 && uniqTargets.length > linkDumpCap)
        ...
        : 1.0;
      const lexicalBase = (0.55 * sourceAlign + 0.45 * selfConsistency + keywordBoost) * penalty * listPenalty;
      const semanticBase = (0.20 * sourceAlign + 0.80 * selfConsistency + (keywordBoost * 0.4)) * listPenalty;
      const baseScore = ((1 - semanticRelaxWeight) * lexicalBase) + (semanticRelaxWeight * semanticBase);
      if (baseScore <= 0.01) continue;

      for (let i = 0; i < uniqTargets.length; i += 1) {
        const tid = uniqTargets[i];
        const lexicalSim = clamp(sourceSims[i], 0, 1);
        const semanticSupport = uniqTargets.length > 1
          ? clamp(targetPeerSums[i] / (uniqTargets.length - 1), 0, 1)
          : clamp(selfConsistency, 0, 1);
        const semanticLift = clamp(semanticSupport - lexicalSim, 0, 1);
        const novelty = semanticLift * (1 - lexicalSim);
        const edgeWeight =
          baseScore *
          (0.45 + 0.35 * lexicalSim + 0.20 * semanticSupport) *
          (1 + semanticRelaxWeight * semanticLift * 0.75);
        ...
        outEdges.push({
          ...
          lexical_sum: lexicalSim,
          semantic_sum: semanticSupport,
          novelty_sum: novelty,
...
async function phaseBuildComponents(ctx) {
  ...
  workDb.exec(`
    ...
    CREATE TABLE edge_pruned (
      ...
      last_time INTEGER,
      avg_lexical_sim REAL NOT NULL DEFAULT 0,
      avg_semantic_support REAL NOT NULL DEFAULT 0,
      avg_novelty REAL NOT NULL DEFAULT 0
    );
  `);

  const pruneInsert = workDb.prepare(`
    INSERT INTO edge_pruned(
      source_story_id,
      target_story_id,
      weight,
      evidence_count,
    ...
    SELECT
      ...
      evidence_count,
      last_time,
      CASE WHEN evidence_count > 0 THEN (lexical_sum / evidence_count) ELSE 0 END AS avg_lexical_sim,
      CASE WHEN evidence_count > 0 THEN (semantic_sum / evidence_count) ELSE 0 END AS avg_semantic_support,
      CASE WHEN evidence_count > 0 THEN (novelty_sum / evidence_count) ELSE 0 END AS avg_novelty
    FROM (
      SELECT
        source_story_id,
        target_story_id,
...
async function phaseEmitArtifacts(ctx) {
  ...
  const manifest = {
    ...
    config: {
      ...
      semantic_relax_weight: args.semanticRelaxWeight,
      surprise_max_lexical: args.surpriseMaxLexical,
      surprise_min_semantic: args.surpriseMinSemantic,
      surprise_min_weight: args.surpriseMinWeight,
      surprise_limit: args.surpriseLimit,
  ...
  if (totalComponents === 0) {
    ...
    await writeJsonMaybeGzip(args.topIndex, { version: 3, generated_at: nowIso(), rows: [], backlinks: [], surprising: [] }, true);
    markPhaseDone(workDb, doneKey);
  ...
  const surprisingRows = [];
  const selectSurprising = workDb.prepare(`
    SELECT
      ep.source_story_id,
      ...
      ep.avg_novelty
    FROM edge_pruned ep
    LEFT JOIN story_profiles src ON src.story_id = ep.source_story_id
    LEFT JOIN story_profiles tgt ON tgt.story_id = ep.target_story_id
    WHERE ep.weight >= ?
      AND ep.avg_lexical_sim <= ?
      AND ep.avg_semantic_support >= ?
    ORDER BY ep.avg_novelty DESC, ep.weight DESC, ep.evidence_count DESC, COALESCE(ep.last_time,0) DESC
    LIMIT ?
  `);
  const rawSurprises = selectSurprising.all(
    args.surpriseMinWeight,
    args.surpriseMaxLexical,
    args.surpriseMinSemantic,
    Math.max(100, Number(args.surpriseLimit || 0) * 3)
  );
  const rankedSurprises = rawSurprises
    .map((row) => {
      const novelty = Number(row.avg_novelty || 0);
      const weight = Number(row.weight || 0);
      const evidenceCount = Number(row.evidence_count || 0);
      const entropyScore = novelty * Math.log1p(1 + evidenceCount) * Math.log1p(Math.max(0, weight) * 12);
      return {
        source_story_id: Number(row.source_story_id),
        source_title: row.source_title || null,
        source_by: row.source_by || null,
        source_time: row.source_time == null ? null : Number(row.source_time),
        source_score: row.source_score == null ? null : Number(row.source_score),
        source_url: row.source_url || null,
        target_story_id: Number(row.target_story_id),
        target_title: row.target_title || null,
        target_by: row.target_by || null,
        target_time: row.target_time == null ? null : Number(row.target_time),
        target_score: row.target_score == null ? null : Number(row.target_score),
        target_url: row.target_url || null,
        weight,
        evidence_count: evidenceCount,
        lexical_sim: Number(row.avg_lexical_sim || 0),
        semantic_support: Number(row.avg_semantic_support || 0),
        novelty,
    ...
    .sort((a, b) =>
      ...
      (b.target_time || 0) - (a.target_time || 0)
    )
    .slice(0, Math.max(0, Number(args.surpriseLimit || 0)));
  for (let i = 0; i < rankedSurprises.length; i += 1) {
    surprisingRows.push({
      rank: i + 1,
      ...rankedSurprises[i]
    });
  ...
  await writeJsonMaybeGzip(args.topIndex, {
    ...
    totals: {
      ...
      surprising: surprisingRows.length
    },
    rows: topRows,
    backlinks: backlinksRows,
    surprising: surprisingRows
  }, true);
...
async function main() {
  ...
  assertFinitePositive("surprise-limit", args.surpriseLimit);
  ...
  assertFiniteNonNegative("surprise-min-weight", args.surpriseMinWeight);
  assertFiniteRange("semantic-relax-weight", args.semanticRelaxWeight, 0, 1);
  assertFiniteRange("surprise-max-lexical", args.surpriseMaxLexical, 0, 1);
  assertFiniteRange("surprise-min-semantic", args.surpriseMinSemantic, 0, 1);
  ...
  try {
    ...
    const hasScoringProgress = getMetaBool(workDb, "phase_score_edges_done") || getMetaBool(workDb, "phase_components_done");
    if (hasScoringProgress && existingSchemaVersion !== RELATED_WORK_SCHEMA_VERSION) {
      setMeta(workDb, "phase_story_profiles_done", "0");
      setMeta(workDb, "phase_score_edges_done", "0");
      setMeta(workDb, "phase_components_done", "0");
      setMeta(workDb, "phase_emit_done", "0");
      setMeta(workDb, "score_last_evidence_id", "0");
      setMeta(workDb, "score_processed_evidence", "0");
      setMeta(workDb, "score_emitted_edges", "0");
      workDb.exec("DELETE FROM edge_scores");
      const fromSchema = existingSchemaVersion || "unversioned";
      progress.done(`Related schema changed (${fromSchema} -> ${RELATED_WORK_SCHEMA_VERSION}); invalidated phases 3/4/5/6`);
    }
